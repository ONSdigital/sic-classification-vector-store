...
# %%
lookup_df = pd.read_csv(f'{data_bucket}Lookup_IT3_Final.csv', 
                        usecols=[
                            'text',
                            'id'
                        ], 
...
# %%
matching_df = pd.read_csv(f'{data_bucket}SAYT_matching.csv', 
                        usecols=[
                            'Correct SIC code',
...
matching_df['Correct SIC code'] = matching_df['Correct SIC code'].apply(lambda x: x if len(x)==5 else f"0{x}")
...
#   model_kwargs={'trust_remote_code': True}
# )
gcpv = GcpVectoriser(project_id=project_id, # Note - this version requires access to a GCP project where VertexAI has been enabled 
...
def collate_matches(vector_store, search_input, out_col_name):
    ...
    search_results['doc_id'] = search_results['doc_id'].apply(lambda x: x if len(x)==5 else f"0{x}")
    search_results_gpby = search_results.groupby('query_id', as_index=False)
    return search_results_gpby.agg(list)[['query_id','doc_id', 'doc_text']].rename(columns={
        'doc_id': out_col_name+'_codes',
        'doc_text': out_col_name+'_descriptions'
        })

hf_matches_full = collate_matches(hf_vs, matching_search_data_full, 'hf_ordered_matches_FULL')
gcp_matches_full = collate_matches(gcp_vs, matching_search_data_full, 'gcp_ordered_matches_FULL')
sayt_matches_full = collate_matches(sayt_vs, matching_search_data_full, 'sayt_ordered_matches_FULL')
hf_matches_partial = collate_matches(hf_vs, matching_search_data_partial, 'hf_ordered_matches_PARTIAL')
gcp_matches_partial = collate_matches(gcp_vs, matching_search_data_partial, 'gcp_ordered_matches_PARTIAL')
sayt_matches_partial = collate_matches(sayt_vs, matching_search_data_partial, 'sayt_ordered_matches_PARTIAL')

# %%
matching_df = pd.merge(matching_df, hf_matches_full, left_on='query_id', right_on='query_id', suffixes=['',''])
matching_df = pd.merge(matching_df, gcp_matches_full, left_on='query_id', right_on='query_id', suffixes=['',''])
matching_df = pd.merge(matching_df, sayt_matches_full, left_on='query_id', right_on='query_id', suffixes=['',''])
matching_df = pd.merge(matching_df, hf_matches_partial, left_on='query_id', right_on='query_id', suffixes=['',''])
matching_df = pd.merge(matching_df, gcp_matches_partial, left_on='query_id', right_on='query_id', suffixes=['',''])
matching_df = pd.merge(matching_df, sayt_matches_partial, left_on='query_id', right_on='query_id', suffixes=['',''])
...
matching_df[useful_cols].to_csv(f'{out_dir}SAYT_semantic_search_results.csv', index=False)
try:
    matching_df[useful_cols].to_parquet(f'{out_dir}SAYT_semantic_search_results.parquet', index=False)
...
for solution in ['hf', 'gcp', 'sayt']:
    ...
    for top_k in [1,3,5,10]:
        ...
        for col in [
            ...
            print(f"{col} accuracy: {(matching_df.apply(lambda x: get_accuracy(x, col, top_k), axis=1)).sum():.3f}%")
# %%
"""
'Hybrid' search idea:
---------------------
